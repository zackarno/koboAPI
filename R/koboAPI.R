  ...
  download_form <- function(formid,
    ...
    if(!is.na(suppressWarnings(as.integer(formid)))){
      old_id <- formid
      formid <- old2new_id(formid, all_forms)
    }else{
      old_id <- new2old_id(formid, all_forms)
    }
  ...
  download_forms_all <- function(user,pwd, api = "https://kobo.humanitarianresponse.info"){
    ...
    form_data <- GET(paste0(api, "/assets/"), authenticate(user, pwd), progress())
    if(form_data$status_code == 403){
    ...
    download_forms_all_df <-  as.data.frame(form_data$results)%>%
  ...
  download_data <- function(formid, user,pwd,
                            api="https://kobo.humanitarianresponse.info",
                            seperator="\\/", all_forms= form_data) {

    if(!exists('pwd') || (pwd == "" || is.null(pwd))){pwd <- readline("Enter password:")}
    if(pwd == "") stop("No password entered.")
    form_data <- download_forms_all(user, pwd, api)

    if(!is.na(suppressWarnings(as.integer(formid)))){
      old_id <- formid
      formid <- old2new_id(formid,all_forms =  form_data)
    }else{
      old_id <- new2old_id(formid, all_forms = form_data)
    }
